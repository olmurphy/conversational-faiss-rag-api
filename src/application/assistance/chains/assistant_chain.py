...
class AssistantChain:
    ...
    def _invoke(self, query, chat_history_messages):
        ...
        if (
            np.max(score[0])
            >= self.app_context.configurations.vectorStore.minScoreDistance
        ):
            retrieved_docs = [
                (selected_docs[i], selected_scores[i]) for i in range(index)
            ]
        context, context_metadata, combine_docs_time = AggregateDocsChain(
            self.app_context
